...
struct CarrierDetector {
    ...
    private static let patterns: [(regex: String, carrier: Carrier, confidence: Double)] = [
        ...
        // 蝦皮店到店：TW + 12-15 位數字 + 可選字母
        // 來源：實際單號 (TW259426993523H)
        (#"^TW\d{12,15}[A-Z]?$"#, .shopee, 0.95),
        ...
        // 7-ELEVEN 交貨便：J + 10-11 位數字
        // 來源：實際單號 (J70974215519)
        (#"^J\d{10,11}$"#, .sevenEleven, 0.9),

        // 7-ELEVEN 賣貨便：N + 10-11 位數字
        // 來源：實際單號 (N01856100569)
        (#"^N\d{10,11}$"#, .sevenEleven, 0.9),

        // 萊爾富：HL + 10-15 位數字（部分單號為純數字，無法從格式區分）
        // 來源：Track.TW 實際單號
        ...
        // 全家店到店：純數字 11 碼（與萊爾富、嘉里、新竹格式重疊，需靠 AI 辨識）
        // 來源：實際單號 (15326511523)
        // 注意：信心度低，純數字格式無法可靠區分物流商
        (#"^\d{11}$"#, .familyMart, 0.4),
